...
export const SERVICE_ERROR_COPY =
  "The Foray service did not answer, so this page has nothing to show. Check your connection and reload.";
...
export function serviceRefusedCopy(status: number, reason?: string): string {
  ...
  if (status === 401 || status === 403) {
    return `The Foray service was reached and asked for an account (HTTP ${status}). This page is meant to be public, so that is our fault, not yours.${said}`;
  }
  return `The Foray service was reached and refused this request (HTTP ${status}), so nothing from it is shown. Your connection is fine.${said}`;
}
...
export const SERVICE_INVALID_COPY =
  "The Foray service answered with something this page could not read, so nothing is shown. That is our bug, not your connection.";
