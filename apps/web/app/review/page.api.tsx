 ...
 *
 * Access is enforced on the SERVER, in the same place the API twin enforces
 * it: `handleModerationCases` runs `withReviewer` (AuthProvider + the
 * AILX_REVIEWERS allowlist). A caller who is not a moderator gets a 404 page,
 * because the queue holds sites nobody has vetted yet — the API twin still
 * answers 401/403, because a script needs to know why it was refused.
 */
...
export default async function ModerationPage({
  ...
  const result = await withApiContext(async (ctx) =>
    handleModerationCases(ctx, await requestHeaderMap(), flat, process.env),
  );
  if (result.status !== 200) notFound();
  const listing = result.body.listing as CaseListing;
  ...
  return (
    <main className="page">
      <div className="container">
        <p className="eyebrow">AILX MODERATION · STAFF ONLY</p>
        <h1 style={{ maxWidth: "22ch" }}>What is waiting, and what we decided.</h1>
