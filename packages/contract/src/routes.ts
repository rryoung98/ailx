 ...
 * which the browser validates the body against (docs/ADR-zod-tanstack.md). `getAttempt` had no caller in `apps/web` and now
 * has one: the report reads the scores of record through it
 * (`apps/web/features/report/useScoresOfRecord.ts`, TEN-128). `countShareView`
 * had none until TEN-146 and now has one (`apps/web/lib/data/shareViews.ts`).
 *
