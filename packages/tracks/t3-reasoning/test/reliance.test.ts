...
import {
  proportionDifferenceInterval, RSR_MIN_SURFACED, wilsonInterval,
} from "../src/scoring.js";
...
const raw = (over: [number, number], under: [number, number]) => ({
  ...
  "rsr.underpowered": over[1] < RSR_MIN_SURFACED ? 1 : 0,
});
...
describe("relianceReportFromRaw", () => {
  ...
  it("says the sitting is underpowered when fewer than 8 plants surfaced", () => {
    ...
    expect(r.underpoweredNote).toContain(`floor for reporting a rate is ${RSR_MIN_SURFACED}`);
  });
