...
- The audit digest content-addresses the TRACK half of `score()` SOURCE at build time (`instruments/demo-2026.1/snapshot.json` `scorers[]`); regenerate with `pnpm --filter @ailx/content-tools run snapshot:demo-2026.1`. The digests are tier-independent — they hash `score()` source, which is the same in both repos. **Read that narrowly:** files inside a track package are hashed by their bytes; `@ailx/core` — which now holds the score allocation, the canonical judgment order, the order-invariant mean/median and `round3` — is recorded by `name@version` only, so a scoring change made there moves the digest only if somebody bumps the version. Bump `packages/core/package.json` whenever scoring-relevant core code changes, and regenerate. See `packages/content-tools/src/scorers.ts` "KNOWN BOUNDARY".
