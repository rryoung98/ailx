...
- [ ] **Close the two remaining audit-digest gaps** (follow-up to the source-addressing work above):
  1. *Cross-package edits are not hashed.* `packages/content-tools/src/scorers.ts` follows RELATIVE imports only; a bare specifier is recorded as `name@<declared range>`, so `@ailx/core@workspace:*` pins nothing and an in-place edit to `packages/core/src/hash.ts` leaves every track digest unchanged. Fix: resolve workspace dependencies to their package directory and recurse into their source closure (they are `workspace:*`, so a directory walk is enough), or pin exact versions and enforce a version bump in CI. Prefer recursion — versions in this repo never move.
  2. *Runtime dependencies are unpinned.* `pnpm-lock.yaml` fixes them for a build, but the digest does not carry a lockfile hash, so "same source, different `three`/`react` patch" is invisible. Fix: hash the lockfile entries reachable from each scorer package into `externals`.
- [ ] **Finish the purity-sandbox move** (FRONTEND.md §9 step 4 leftovers): `apps/web/lib/instrument.ts` (snapshot → track config derivation, T2 deck sampling, answer keys) and `apps/web/lib/validateChecks.ts` (the §14 self-audit the `/validate` page renders) are still app-local. Both are blocked on one thing: `instrument.ts` calls `assetUrl()` from `lib/mode.ts`, which reads `NEXT_PUBLIC_BASE_PATH`. Fix: split the pure derivation into `packages/report` (or `@ailx/content-tools`) taking asset resolution as an injected `(path: string) => string`, and leave the Next-specific binding in `apps/web`. `lib/registry.ts` stays in the app: it dynamic-imports React Runners.
