...
- **Humans are worse at detection than they believe.** In a 2,000-person study, 0.1% correctly classified every item, and confidence stayed above 60% regardless of correctness. Overconfidence, not accuracy, is likely to be Foray's most quotable finding.
...
| **Foray**                 | **Instrument**         | **Adults, 3 languages** | **4 authentic performance tracks**      | **Pilot Aug 2026**                 |
...
What Foray claims to measure, stated tightly enough to be falsifiable, and mapped onto the frameworks reviewers will already know.

Foray defines **applied AI literacy** as a person's capacity to produce good outcomes in an information environment saturated with generative systems. It resolves into four measurable capabilities, each of which is a track.
...
NIST AI 800-2 asks benchmark authors to say which kind of claim they are making. Foray declares its levels rather than leaving them to be inferred:
...
>
> Human accuracy on synthetic-image detection moves from **72% at 1 second to 82% at 20 seconds**. A swipe interface is structurally a snap-judgement instrument and suppresses accuracy by roughly ten points against unlimited viewing. Foray chooses snap judgement deliberately — it is the condition under which people actually encounter this content — holds exposure constant across all items, and states the choice in every report. An undeclared exposure time would make cross-year comparison meaningless.
...
>
> The assistant's environment is **seeded with known-incorrect outputs** at predetermined points — a misattributed figure, a plausible but false causal claim, a fabricated citation, a subtly wrong calculation. Scoring then measures two behavioural rates: how often the candidate accepted a seeded wrong output (**over-reliance**) and how often the candidate refused correct, source-grounded advice (**under-reliance**). Both are objective and cannot be gamed by writing prettier prose. The two SCORED components are named for what the candidate did — `errorCatchRate` and `adviceUptakeRate` — because a component holds the points earned, not the failure avoided. They were `rsr` and `rair` until 2026-09-02, after the appropriate-reliance literature, and that was wrong: they are **not** the published statistics of that literature (TEN-38). They were then `overReliance` and `underReliance` for a day, and that was wrong the other way: a candidate who caught every plant scored 50 out of 50 on a field called `overReliance` (TEN-72). The two failure RATES keep the literature's names, in the raw record as `reliance.over` and `reliance.under`. See "Stated against our own case" below.

### Score allocation

**160 points, and the track has a new named construct: calibrated reliance — knowing when to use the model and when not to.**

- **50 pts — Planted errors caught (the `errorCatchRate` component).** Did the candidate catch and reject the seeded wrong outputs? Fully objective. The reporting floor is **eight planted errors** — what the published design behind the construct used — and **the released form in 2026.1 plants four, the operational form fewer still**, so every sitting on them is flagged `errorCatchRate.underpowered` and its rate is reported as provisional (TEN-91; "Why the floor is eight and the form deals four" below)
- **30 pts — Correct advice taken up after deliberation (the `adviceUptakeRate` component).** Did the candidate take correct, source-grounded advice *after deliberating on it*? A blind instant accept earns half credit
...
Every existing AI-literacy instrument, Foray included until now, measures competence *conditional on using AI*. None measures whether the person should have opened the model at all. The failure mode that matters in real organisations is reflexive delegation of judgement.
...
5. **Not "50 + 30 points is a measurement".** Eight planted errors give a rate with a ±0.35
   interval, and the four the released form actually plants give about ±0.49. The rates are
   reported as bands with intervals until the retest in `docs/TRANSFER-STUDY.md` §3.2 has
   been run.
...
10. **Not "the shipped form has enough plants to report a rate".** The released form has four
    — the operational one fewer — against a floor
    the same evidence sets at eight (Schemmer, Kühl, Benz & Satzger 2022, arXiv:2204.06916:
    8 incorrect + 8 correct items per condition). Four is what the instrument deals, not what
    the evidence supports, and declaring eight against those forms made the package refuse to
    open (TEN-73, TEN-91). Until the forms carry
    eight, every T3 rate in this instrument is an underpowered estimate and is labelled one.
...
The evidence, by name and number. Rosbach, Ammeling, Ganz, Bertram, Conrad, Riener and Aubreville (MELBA 2026, DOI 10.59275/j.melba.2026-87b1, arXiv:2603.11821) measured normalised weight-of-advice in 28 pathology experts: 0.48 without time pressure, 0.54 with it, t(27) = 2.55, p = .017, and accuracy was worse under pressure (mean absolute deviation 19.42 against 27.79). That supports the direction, on a modest effect in a small expert sample. The earlier study by the same group (arXiv:2411.00998) found the **frequency** of automation bias unchanged under a 10-second countdown, p = 0.19; only its severity moved. So Foray may not say that a clock makes candidates adopt more errors. Swaroop, Buçinca, Gajos and Doshi-Velez (arXiv:2306.07458) found that **when** the assistant speaks moved over-reliance more than how long the participant had, and Buçinca, Malaya and Gajos (arXiv:2102.09692, n = 199) found verification effort is a property of the interface as much as of the person.

**The confound, stated before anyone else states it.** Foray varies the clock and the interface together. Until the timed/untimed arm in `docs/TRANSFER-STUDY.md` §3.5 runs — same document, same assistant, same planted-error set, timer the only thing that moves — every "under time pressure" sentence in a report describes this form, not the construct. The report may name the condition a sitting ran under. It may not attribute the difference to time pressure alone.

### Why the floor is eight and the form deals four

The over-reliance component carries 50 of 160 points and **its item count is the number of planted errors the form surfaces.** Four cannot support that weight: catching 2 of 4 versus 3 of 4 is a 12.5-point difference decided by essentially one event, and a four-item subtest cannot have usable reliability at any weight. Eight is the declared floor (`ERROR_CATCH_MIN_SURFACED`), and it is what the evidence supports — the published design this construct comes from ran **8 incorrect + 8 correct items per condition** (Schemmer, Kühl, Benz & Satzger 2022, arXiv:2204.06916; recorded in the private repo's `docs/EVIDENCE-CALIBRATED-RELIANCE.md` §1.2 as SUPPORTING the eight-plant floor and as CONTRADICTING any individual-score claim built on it, since those 8 supported a group mean over 200 participants).

**The released form plants four, and the instrument declares four (TEN-91).** The released T3 form deals four plants, one instance of each error family; the operational form deals fewer and declares what it deals. TEN-73 raised the *declaration* to eight to match a browser form that had been extended to eight, which left the declaration disagreeing with the operational form; the load guard then refused the package and the released tier would not open. Closing that gap the other way means authoring four more plants in both tiers — content work with its own review — so the number written down is the number dealt, and the shortfall is reported rather than hidden.

**What four costs, stated where a reader meets it.** A rate from four opportunities has a 95% interval of roughly ±0.49. 2 of 4 is 0.50 (0.15 to 0.85); 3 of 4 is 0.75 (0.30 to 0.95). The intervals overlap almost entirely, and 12.5 of the 50 planted-error points move on one event. So every sitting on a 2026.1 form is flagged `errorCatchRate.underpowered` in the record, and the report names how many plants surfaced, names the floor, works the ±interval from the count the sitting actually had, and tells the candidate to treat the rate and the band as provisional. **The floor is not lowered to four**: lowering it would make an underpowered sitting look powered, which is the one thing this decision must not buy.

Eight also fits the re-versioning economics. The plants are instances of four stable error **families** — misattributed figure, false causal claim, fabricated citation, wrong calculation. Families are stable even as instances burn, so a re-version is new instances of known families, which is the cheapest possible refresh in the instrument. Raising a form from four to eight is a second instance of each family, and it must move both tiers together or the package stops opening again.
...
> **Stated against our own case.** The one piece of external evidence that cuts the other way is the PISA 2029 Media & AI Literacy framework, which gives roughly half its test time to *create* alongside analyse and evaluate. Cutting a generative track moves Foray away from that balance. The answer is that T1 is a create track, it is now the flagship at 160 points, and it is the create track that has an external criterion. Foray has not stopped measuring creation; it has stopped measuring it twice.
...
> **Foray takes the AP model with ARC-AGI's calibration discipline — not the IMO's**
>
> The IMO analogy is right for the *feel* of the exam and wrong for its measurement design. IMO makes no attempt to equate across years; it achieves comparability purely by rank-based quota. Foray cannot do that and also claim a stable standard. So: **Year 1 is norm-referenced by quota** (a calibration cohort leaves no alternative), while the **secure anchor block is built from day one** so that Year 2 onward can be equated. The standard, once set by Angoff, is held fixed and the raw cut floats — the AP model. And every new annual form is human-calibrated against the prior form's difficulty before release, the ARC-AGI way.
...
Foray adopts **NIST AI 800-2, *Practices for Automated Benchmark Evaluations*** (CAISI, January 2026) as the document's spine, and says so explicitly. It is the most reviewer-legible framing available and it addresses LLM-as-a-judge directly. The four practices are mirrored throughout:
...
| Bias                                               | Magnitude                                                                                                                                                           | Mitigation in Foray                                                                                                   |
|----------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------|
...
| T3    | **Planted errors as traps**, revealed at the end: "you caught 3 of 4"                 | The reveal happens after submission. The traps are already the objective core of the track (§T3); naming them at the end costs nothing and produces the exam's best story.                                           |
...
- Schemmer, Kühl, Benz, Bartos & Satzger, [Appropriate Reliance on AI Advice: Conceptualization and the Effect of Explanations](https://dl.acm.org/doi/10.1145/3581641.3584066), IUI 2023 — the definitions of RAIR and RSR Foray names its components after and does **not** compute ([preprint](https://arxiv.org/abs/2302.02187))
